...
public class HostCommand extends HyriCommand<HyramePlugin> {
    ...
    private static final Function<Player, BaseComponent[]> HELP = player -> new CommandHelpCreator("h", "host", player)
    ...
    public void handle(HyriCommandContext ctx) {
        ...
        this.handleArgument(ctx, "kick %player_server%", output -> {
            ...
            HyriAPI.get().getServerManager().sendPlayerToLobby(target.getUniqueId());
        ...
        this.handleArgument(ctx, "banlist", output -> {
            ...
            for (UUID bannedPlayer : account.getPlayersBannedFromHost()) {
                builder.append(" ").append(Symbols.DOT_BOLD).color(ChatColor.DARK_GRAY).append(" ").append(HyriAPI.get().getPlayerManager().getPrefix(bannedPlayer)).append("\n");
            }
        ...
        this.handleArgument(ctx, "oplist", output -> {
            ...
            for (UUID host : hostData.getSecondaryHosts()) {
                builder.append(" ").append(Symbols.DOT_BOLD).color(ChatColor.DARK_GRAY).append(" ").append(HyriAPI.get().getPlayerManager().getPrefix(host)).append("\n");
            }
        ...
        this.handleArgument(ctx, "wl list", output -> {
            ...
            for (UUID whitelistedPlayer : hostData.getWhitelistedPlayers()) {
                builder.append(" ").append(Symbols.DOT_BOLD).color(ChatColor.DARK_GRAY).append(" ").append(HyriAPI.get().getPlayerManager().getPrefix(whitelistedPlayer)).append("\n");
            }
        ...
        this.handleArgument(ctx, "give %input% %integer%", output -> {
            ...
            if (material == null) {
                player.sendMessage(ChatColor.RED + HyriCommonMessages.INVALID_ARGUMENT.getValue(player) + materialName + ".");
                return;
            }
