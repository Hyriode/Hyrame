...
public class Utils {
    ...
    public static ArrayList<Team> createClassicTeams(int nbTeam, int teamSize, Game game) {
        ArrayList<Team> teams = new ArrayList<>();
        for (int i = 0; i < nbTeam; i++) {
            final Team team = new Team(game ,TeamColor.getValueByID(i), null, teamSize, false);
            teams.add(team);
        }
        return teams;
    }

    public static ArrayList<Team> createClassicTeams(int nbTeam, int teamSize, ArrayList<Player> players, Game game) {
        if(players.size() == nbTeam * teamSize) {
            ...
            for (int i = -1; i < nbTeam; i++) {
                ...
                final Team team = new Team(game, TeamColor.getValueByID(i), players1, teamSize, false);
                teams.add(team);
            }
            return teams;
        }else {
