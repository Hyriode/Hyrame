 ...
 */
public class ItemHandler extends HyriListener<HyramePlugin> {

    public ItemHandler(HyramePlugin plugin) {
        super(plugin);
    }
    ...
    private HyriItem<?> checkItem(ItemStack itemStack) {
        ...
        if (nbt.hasTag(ItemManager.ITEM_NBT_KEY)) {
            return this.plugin.getHyrame().getItemManager().getItem(nbt.getString(ItemManager.ITEM_NBT_KEY));
        }
        return null;
    }
